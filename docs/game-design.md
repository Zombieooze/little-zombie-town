...
- **Residential Zone Pass 1 is complete.** The northeast Residential Zone now has a handcrafted abandoned neighborhood block using no-roof, wall-only house construction so interiors stay visible from gameplay. The pass added a mix of single-story houses, two-story houses, and a small townhouse/duplex-style building with passable doorway/garage openings, believable upper-body/head-height windows, simple floor surfaces, lightweight interior room divisions, and a few furniture hints.
...
- Residential wall pieces, interior dividers, fences/hedges, vehicles, trees, shed, bench, and picnic table register through the existing world collider/safe-spawn system where appropriate, so pickup and medkit safe-spawn checks avoid the new solid residential obstacles without adding a new save or spawn system. Other zones remain pending future art/build passes.

...
- Only abilities marked `implemented: true` can appear in the level-up card pool. Six special abilities are currently implemented; removed or future roadmap abilities must stay out of the implemented pool until their behavior is intentionally added.
...
Six special abilities are implemented and can appear in the level-up card pool, while the player can still choose only 4 special abilities per run:
...
6. **Scrap Turret:** An automatic deployable turret special ability that builds a small temporary scrap turret near the player on cooldown. The turret holds an area by facing and shooting nearby zombies with scrap bolts, counts toward the 4 special ability limit, and upgrades from Level 1 to Level 10 through damage, fire-rate, range, duration, active-turret count, and stronger scrap fire. Scrap Turret uses the normal zombie damage/death/reward path so turret kills still award XP and coins.

With six special abilities implemented, the 4-ability cap remains the build constraint: players can unlock only four in one run while already chosen abilities and passive upgrades keep appearing normally.
...
- **Heavy Hitter** (`dmg`, 💪): max 10, base cost 30, +5% damage per level. This applies to the bat and player-created abilities/projectiles where practical, but not enemy damage.
...
- **Springy Boots** (`stamina`, 🥾): max 8, base cost 30, +5% jump strength per level. The saved `stamina` id is retained for compatibility, but the upgrade now affects jump height instead of stamina.
...
- The arena is **130 x 130** with one clear central road crossing/intersection and four compact themed corners: Residential, Gas / convenience store, Old Fountain Park, and an abandoned Motel zone. A light whole-map dressing pass removed temporary floating zone labels, cleaned stray center-road sidewalk/marking clutter, and added street lamps, roadside trees, and a few small abandoned-town props while preserving combat readability.
...
- Active major blockers are intentionally limited to solid buildings, burnt vehicles, the solid motel, and larger park trees. Light rubble patches, road cracks/markings, sidewalks, parking lines, pumps, street lamps, signs, hydrants, utility poles, trash, tire/scrap piles, and small ground debris are decorative or minimally intrusive.
...
- Vehicles are solid obstacles with footprint colliders and are placed lightly around believable spots such as the motel parking lot, road edges, the gas station area, and residential driveways. They should add abandoned-town flavor and route variety while preserving open combat space and avoiding maze-like clutter.
- The old parking lot / junk lot corner has been replaced with an abandoned motel zone. The motel is an L-shaped, two-story, solid flat-roof building that hugs the outside south/east edges while opening inward toward a cleaner inner parking lot. Lower motel fronts show room doors, upper fronts show dark, boarded, or broken windows, and a roadside **MOTEL** sign makes the landmark readable from the road.
- The motel zone is scenery and combat-friendly obstacle space: the building wings, burnt vehicles, and sign provide a few solid obstacles, while parking stall markings, driveway pavement, and light rubble remain non-blocking so the player and zombies have room to fight in the lot.
- The park corner is now an **Old Fountain Park**: a small abandoned open-grass park built around a low-poly broken dry fountain centerpiece, clear simple paths to nearby road/sidewalk edges, sparse larger trees around the outer park, extra worn/dead grass patches, light cracked-stone/rubble detail, and a couple of burnt vehicles placed near park/road edges. The fountain, larger tree trunks, and vehicles are the only meaningful park blockers; path surfaces, worn ground, and rubble details remain non-blocking so the area stays combat-friendly and avoids clutter.
- Some reusable world prop helpers remain in `src/world.js` for now because they are self-contained prefab code and may still be useful later, but the simplified arena avoids clutter-heavy helpers in the active world. The current light dressing pass uses roughly ten non-blocking street lamps, additional sidewalk/roadside trees, and a small scatter of signs, hydrants, utility poles, trash, tire/scrap piles, rubble, and burnt vehicles to sell the abandoned-town mood without adding tight choke points.

...
function createPostSign(options = {}, type = 'stop') {
  ...
  return finishAsset(g, tuneAssetScale(options, { lamp: 1.28, street: 1.3, stop: 1.3, bus: 1.15 }[type] ?? 1), ['lamp','street','bus','stop'].includes(type) ? { type: 'circle', radius: .28, label: `${type}-post` } : null);
}
...
export function createUtilityPole(options = {}) { const g=new THREE.Group(); assetPart(g, box(.28,3.0,.28,0x6b5138), [0,1.5,0]); assetPart(g, box(1.8,.18,.18,0x6b5138), [0,2.65,0]); assetPart(g, new THREE.Mesh(new THREE.CylinderGeometry(.22,.22,.55,8), ASSET_MATS.metal), [-.55,1.6,0]); return finishAsset(g, tuneAssetScale(options, 1.35), {type:'circle', radius:.32, label:'utility-pole'}); }
...
export function createBench(options={}) { const g=new THREE.Group(); assetPart(g,box(2,.18,.45,0x55603f),[0,.55,0]); assetPart(g,box(2,.18,.35,0x55603f),[0,.95,.25],[-.25,0,0]); for(const x of [-.75,.75]) assetPart(g,box(.16,.55,.16,0x5a5147),[x,.28,0]); return finishAsset(g, tuneAssetScale(options, 1.12), {type:'rect', width:2.1, depth:.75, label:'bench'}); }
export function createPicnicTable(options={}) { const g=new THREE.Group(); assetPart(g,box(2.2,.18,.8,0x6b5138),[0,.8,0]); for(const z of [-.75,.75]) assetPart(g,box(2.1,.16,.35,0x6b5138),[0,.48,z]); for(const x of [-.8,.8]) assetPart(g,box(.16,.75,.16,0x5a5147),[x,.38,0],[0,0,.25]); return finishAsset(g, tuneAssetScale(options, 1.12), {type:'rect', width:2.4, depth:1.9, label:'picnic-table'}); }
...
export function createPlaygroundSlide(options={}) { const g=new THREE.Group(); assetPart(g,box(.75,.12,2.4,0xb84e36),[0,.65,0],[.55,0,0]); assetPart(g,box(.12,1.4,.12,0x3f4548),[-.42,.7,-.75]); assetPart(g,box(.12,1.4,.12,0x3f4548),[.42,.7,-.75]); return finishAsset(g, tuneAssetScale(options, 1.28), {type:'rect', width:1.1, depth:2.4, label:'playground-slide'}); }
export function createPlaygroundSwings(options={}) { const g=new THREE.Group(); for(const x of [-1.2,1.2]) { assetPart(g,box(.12,2.2,.12,0x6b5138),[x,1.1,-.55],[0,0,.22*x]); assetPart(g,box(.12,2.2,.12,0x6b5138),[x,1.1,.55],[0,0,-.22*x]); } assetPart(g,box(2.8,.14,.14,0x6b5138),[0,2.15,0]); for(const x of [-.55,.55]) assetPart(g,box(.55,.08,.35,0x252525),[x,.55,0]); return finishAsset(g, tuneAssetScale(options, 1.25), {type:'rect', width:3, depth:1.4, label:'playground-swings'}); }

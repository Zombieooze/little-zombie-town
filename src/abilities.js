...
export const BASEBALL_BAT_ID = 'baseballBat';
export const BASEBALL_BAT_MAX_LEVEL = 8;

const REMOVED_ABILITY_IDS = new Set(['bearTrapToss', 'shockwaveStomp']);

export function isRemovedAbilityId(id) {
  return REMOVED_ABILITY_IDS.has(id);
}

export function isActiveAbilityId(id) {
  return Boolean(ABILITY_DEFINITIONS[id]?.implemented) && !isRemovedAbilityId(id);
}

export function sanitizeAbilityState(state) {
  const abilities = state?.abilities;
  if (!abilities) return;
  if (!Array.isArray(abilities.chosen)) abilities.chosen = [BASEBALL_BAT_ID];
  abilities.chosen = abilities.chosen.filter(isActiveAbilityId);
  if (!abilities.chosen.includes(BASEBALL_BAT_ID)) abilities.chosen.unshift(BASEBALL_BAT_ID);
  abilities.chosen = [...new Set(abilities.chosen)].slice(0, MAX_SPECIAL_ABILITIES);
  abilities.levels = Object.fromEntries(
    Object.entries(abilities.levels ?? {}).filter(([id]) => isActiveAbilityId(id) && abilities.chosen.includes(id)),
  );
  abilities.levels[BASEBALL_BAT_ID] = Math.max(1, abilities.levels[BASEBALL_BAT_ID] ?? 1);
}

export function getBaseballBatStats(level) {
  const safeLevel = Math.max(1, Math.min(BASEBALL_BAT_MAX_LEVEL, Math.floor(Number(level) || 1)));
  return {
    damage: 14 + safeLevel * 8,
    range: 3.4 + safeLevel * 0.5,
    cooldown: 1.7 - safeLevel * 0.12,
  };
}

export function getAbilityMaxLevel(id) {
  return ABILITY_DEFINITIONS[id]?.maxLevel ?? MAX_ABILITY_LEVEL;
}
...
export const ABILITY_DEFINITIONS = {
  baseballBat: {
    ...
    unlockDescription: 'Your trusty starting bat.',
    maxLevel: BASEBALL_BAT_MAX_LEVEL,
    implemented: true,
    defaults: getBaseballBatStats(1),
    upgrades: {
...
export function getImplementedAbilityDefinitions() {
  return Object.values(ABILITY_DEFINITIONS).filter((ability) => isActiveAbilityId(ability.id));
}
...
export function getActiveAbilityIds(state) {
  sanitizeAbilityState(state);
  return state.abilities?.chosen ?? [];
}

function makeAbilityState() {
  return {
    chosen: [BASEBALL_BAT_ID],
    levels: { [BASEBALL_BAT_ID]: 1 },
    baseballBat: { ...ABILITY_DEFINITIONS.baseballBat.defaults },
...
export function resetAbilities(scene, state) {
  ...
  applyBaseballBatStats(state, 1);
}

export function isAbilityUnlocked(state, id) {
  sanitizeAbilityState(state);
  return isActiveAbilityId(id) && (state.abilities?.chosen?.includes(id) ?? false);
}

export function getAbilityLevel(state, id) {
  sanitizeAbilityState(state);
  return isActiveAbilityId(id) ? (state.abilities?.levels?.[id] ?? 0) : 0;
}
...
export function unlockAbility(scene, state, id, player) {
  sanitizeAbilityState(state);
  if (!isActiveAbilityId(id) || isAbilityUnlocked(state, id) || !canUnlockMoreAbilities(state)) return;
...
export function applyAbilityUpgrade(scene, state, id, player) {
  ...
  if (!isActiveAbilityId(abilityId) || !isAbilityUnlocked(state, abilityId)) return;
...
function applyBaseballBatStats(state, level) {
  const stats = getBaseballBatStats(level);
  state.pulseDamage = stats.damage;
  state.pulseRange = stats.range;
  state.pulseCooldown = Math.max(0.1, stats.cooldown * Math.max(0.05, state.cooldownMultiplier ?? 1));
  if (state.abilities?.baseballBat) Object.assign(state.abilities.baseballBat, stats);
}

function applyAbilityLevelTuning(state, abilityId, level) {
  const abilities = state.abilities;
  if (abilityId === BASEBALL_BAT_ID) {
    applyBaseballBatStats(state, level);
  }
